...
class UberReceiptProcessor:
    ...
    def extract_info_from_image(self, image_path):
        ...
        if "eats" in clean_text:
            ...
            receipt_type = "Trip"
            date_pattern = r"(\d{1,2}\s*月\s*\d{1,2}\s*日\s*\d{1,2}:\d{2}[ap]m)"
            date = re.search(date_pattern, clean_text)
            if date:
                date_chinese = date.group(1).strip().replace(" ","")
                date_chinese = "24年" + date_chinese
                print(date_chinese)
                date_obj = datetime.strptime(date_chinese,"%y年%m月%d日%I:%M%p")
